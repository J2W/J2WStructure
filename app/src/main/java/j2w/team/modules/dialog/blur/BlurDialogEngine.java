...
public class BlurDialogEngine {
	...
	/**
	 * Since image is going to be blurred, we don't care about resolution. Down
	 * scale factor to reduce blurring time and memory allocation.
	 */
	...
	/**
	 * Factor used to down scale background. High quality isn't necessary since
	 ...
	 * Resume the engine.
	 *
	 * @param retainedInstance
	 *            use getRetainInstance.
	 */
	 ...
	 */
	@SuppressLint("NewApi") public void onDismiss() {
		// remove blurred background and clear memory, could be null if
		// dismissed before blur effect
		// processing ends
		// cancel async task
		mBluringTask.cancel(true);
		if (mBlurredBackgroundView != null) {
			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
				mBlurredBackgroundView.animate().alpha(0f).setDuration(mAnimationDuration).setInterpolator(new AccelerateInterpolator()).setListener(new AnimatorListenerAdapter() {
	 ...
	 * <p/>
	 * Higher down scale factor will increase blurring speed but reduce final
	 * rendering quality.
	 *
	 * @param factor
	 *            customized down scale factor, must be at least 1.0 ( no down
	 *            scale applied )
	 */
	...
	private void blur(Bitmap bkg, View view) {
		...
		// define layout params to the previous imageView in order to match its
		// parent
		...
		// evaluate top offset due to action bar, 0 if the actionBar should be
		// blurred.
		int actionBarHeight;
		...
		// check if status bar is translucent to remove status bar offset in
		// order to provide blur
		...
		// add offset to the source boundaries since we don't want to blur
		// actionBar pixels
		Rect srcRect = new Rect(0, topOffset, bkg.getWidth() - rightOffset, bkg.getHeight() - bottomOffset);

		// in order to keep the same ratio as the one which will be used for
		// rendering, also
		...
		try {
			if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB || mHoldingActivity instanceof ActionBarActivity) {
				// add offset as top margin since actionBar height must also
				// considered when we display
			...
			// no dependency to appcompat, that means no additional top offset
			// due to actionBar.
		...
		// draw background from source area in source background to the
		// destination area on the overlay
		canvas.drawBitmap(bkg, srcRect, destRect, paint);

		// apply fast blur on overlay
		// if (mUseRenderScript) {
		// overlay = RenderScriptBlurHelper.doBlur(overlay, mBlurRadius, true,
		// mHoldingActivity);
		// } else {
		...
		if (mDebugEnable) {
			...
			Log.d(TAG, "Allocation : " + bkg.getRowBytes() + "ko (screen capture) + " + overlay.getRowBytes() + "ko (blurred bitmap)"
	...
	private class BlurAsyncTask extends AsyncTask<Void, Void, Void> {
		...
		@Override protected void onPreExecute() {
			...
			/**
			 * After rotation, the DecorView has no height and no width.
			 * Therefore .getDrawingCache() return null. That's why we have to
		...
		@Override @SuppressLint("NewApi") protected void onPostExecute(Void aVoid) {
			super.onPostExecute(aVoid);

			mHoldingActivity.getWindow().addContentView(mBlurredBackgroundView, mBlurredBackgroundLayoutParams);

			if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB_MR1) {
				mBlurredBackgroundView.setAlpha(0f);
				mBlurredBackgroundView.animate().alpha(1f).setDuration(mAnimationDuration).setInterpolator(new LinearInterpolator()).start();
			}
